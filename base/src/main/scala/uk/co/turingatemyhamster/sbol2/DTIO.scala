...
object DTIO {
  ...
  DT <: Datatree with Relations](s2: S2, dt: DT) = new Object {
    def apply(sd: s2.SBOLDocument)(
      implicit i2uri: s2.URI => dt.URI, q2name: s2.QName => dt.Name): dt.DocumentRoot =
    {
      ...
      implicit val s2impl = new s2.ToImplicits[dt.URI, dt.Name, dt.PropertyValue] {
    ...
    def apply(doc: dt.DocumentRoot): s2.SBOLDocument = {
      implicit val s2impl = new s2.FromImplicits[dt.URI, dt.Name, dt.PropertyValue] {
